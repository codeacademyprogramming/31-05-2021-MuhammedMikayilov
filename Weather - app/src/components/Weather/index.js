...
function Weather() {
  ...
  const handleRemove = React.useCallback(
    (item) => {
      setState(state.filter((city) => city.data.name !== item.name));
    },
    [state]
  );
  ...
  const handleSubmit = React.useCallback(
    (e) => {
      ...
      weatherService
        ...
        .then(({ data }) => {
          let finded = state.find(
            (st) => st.data.name.toLowerCase() === inputVal.toLowerCase()
          );
        ...
        .catch(() => swal("NOT FOUND", "We cannot found this city", "error"));
  ...
  return (
    <Container style={{ marginTop: "10%" }}>
      ...
      <Row>
        ...
        {state.map((city) => (
          <Col sm="3" key={city.data.name} className="mt-5">
            <Card>
              <CardImg
                top
                width="100%"
              ...
              <CardBody>
                ...
                <CardTitle tag="h5">
                  {handleDegree(city.data.main.temp)}
                </CardTitle>
                <CardText>
                  Sunrize:
                  {new Date(city.data.sys.sunrise).toLocaleTimeString()} <br />
                  Sunset: {new Date(city.data.sys.sunset).toLocaleTimeString()}
                </CardText>
                <Button
                  className="bg-transparent btn-outline-danger"
                  onClick={() => handleRemove(city.data)}
                >
                  Remove
                </Button>
              </CardBody>
